...
let cityArr = cities.filter((i) => i.charAt(i.length - 1) === "i")
// console.log(cityArr)
...
let findColor = fruits.forEach((element) => {
  if (element.color === "red") {
    return console.log(`The fruit with index ${fruits.indexOf(element)} is an apple`)
  } else if (element.color === "orange") {
    return console.log(`The fruit with index ${fruits.indexOf(element)} is an orange`)
  } else {
    return console.log(`The fruit with index ${fruits.indexOf(element)} is neither apple or orange.`)
  }
...
let totalRicePrice = foods.filter((tag) => tag.tags.includes("rice")).reduce((acc, curr) => acc + curr.price, 0)
